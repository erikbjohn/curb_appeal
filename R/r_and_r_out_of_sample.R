r_and_r_out_of_sample <- function(){
  dt_r_and_r_out_of_sample_location <- '~/Dropbox/pkg.data/curb_appeal/jrefe/r_and_r/out_of_sample.rds'
  if(!file.exists(dt_r_and_r_out_of_sample_location)){
    ...
    lapply(f_list, function(x) file.copy(x, stringr::str_replace(x, '(?<=r\\_and\\_r\\/).+(?=\\/own)', 'out_of_sample_to_score')))
    l_scores <- list.files('~/Dropbox/pkg.data/curb_appeal/jrefe/r_and_r/out_of_sample_scores/', full.names=TRUE)
    ...
    saveRDS(dt_classify, r_and_r_out_of_sample_location)
  } else {
    dt_r_and_r_out_of_sample <- readRDS(dt_r_and_r_out_of_sample_location)
  }
  return(dt_r_and_r_out_of_sample)
}