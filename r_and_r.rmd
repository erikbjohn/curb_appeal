...
The model files for replication as well as the python code are saved on our publically accesible dropbox page at \url{https://www.dropbox.com/sh/b5mhoovcqi9wjn1/AAD2W3NB5opvmrQDvayHKglka?dl=0}. The photos used for training may be found at \url{https://www.dropbox.com/sh/vatw6z3cc625q2a/AABGU3N6GKq2LRSStcBR_yCGa?dl=0}. We provide links to these sources in the paper. Importantly, this includes all code and data needed to replicate the training and model creation as well as the model itsself.
...
Class diagnostics are provided in Table \ref{tbl:in_sample_class} and in the manuscript.
...
### 
```{r, eval=FALSE }
source('R/r_and_r_out_of_sample.R')
dt <- r_and_r_out_of_sample()
l <- caret::confusionMatrix(as.factor(as.character(dt$max_val_predict)),
                           as.factor(dt$ground_truth_score))
```
